...
async function buildDoc({date,reportNo,supervision,labors,pkg,activities,nextDay,photos}){
  const ch=[];
  ...
  for(let i=0;i<photos.length;i+=2){
    ...
    for(let j=i;j<Math.min(i+2,photos.length);j++){
      ...
      cells.push(new TableCell({borders:NBDS,width:{size:photoW,type:WidthType.DXA},
        ...
        children:[new Paragraph({alignment:AlignmentType.CENTER,
          children:[new ImageRun({data:buf,type:ext==='jpg'?'jpeg':ext,transformation:{width:250,height:188}})]})]
      }));
    }
...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    const docBuffer=await buildDoc({date,reportNo,supervision,labors,pkg,activities,nextDay,photos:photoPaths});
    res.setHeader('Content-Type','application/vnd.openxmlformats-officedocument.wordprocessingml.document');
    res.setHeader('Content-Disposition',`attachment; filename="DPR_${reportNo}.docx"`);
    res.send(docBuffer);
